...
export default class Game extends Component {
    ...
    componentDidMount(){
        this.selectAll();
        //用window.requestAnimationFrame代替setInterval
        this.myReq=window.requestAnimationFrame(()=>{
            this.draw();
        });
        // setInterval(()=>{
    ...
    timeButton=()=>{//计时器开关
        if (this.state.on) {
            ...
            if (this.state.time>10){
                ...
                mosquito.top=-100;
                mosquito.left=Math.random()*window_innerWidth;
                balls.map((item)=>{
                    item.top=-100;
                    item.left=-100;
                    return null;
            ...
            this.timer = setInterval(() => {
                if(this.state.touch){
                    alert("游戏结束！分数"+this.state.time+"!\n" +
                        "提示：鼠标控制小格，躲避怪兽们的攻击！坚持时间越长分数越高。" +
                        "小格很脆弱，请保护好它。小格被怪物袭击之后分数将会停止增加，就只能重新开始咯！\n" +
                        "空格：暂停/重新开始\n" +
                        "F：小格的技能（注意冷却槽哦）");
    ...
    checkCircle=(obj1, obj2)=>{
        ...
        let r1=this.radius(obj1height,obj1width);
        let r2=this.radius(obj2height,obj2width);
    ...
    submitS=()=>{
        const url=global.localhostUrl+"game1/submit";
        fetch(url, {
            method: "POST",
            mode: "cors",
            credentials:"include",//跨域携带cookie
    ...
    selectAll=()=>{
        const url=global.localhostUrl+"game1/selectAll";
        fetch(url, {
            sw:false,
            method: "POST",
            mode: "cors",
    ...
    render() {
        return (
            <div className={"demo3"}>
                <div style={{
    width: (this.state.f_time) * 10,
    height: 10,
    backgroundColor: this.state.f_time < 20 ? this.state.f_time < 10 ? "#ff3333" : "#ffdd66" : "#99ff99"
}}/>
                ...
                <br/>
                {
                    this.state.scoreList.length===0?"离线模式":this.state.scoreList.map((item,i)=>{
                        if(window_innerHeight<800 && i>9){
                            return null
                        }
                        return <div key={i} style={{textAlign:"center"}}>
                            <div >
                                {i+1}.{item.username+" "+item.score+"分"}
                            </div>
                        </div>
                    })
                }
